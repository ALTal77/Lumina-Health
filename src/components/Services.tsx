        ...
        {selectedService && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <button
                onClick={() => setSelectedService(null)}
                className="absolute top-4 end-4 p-2 rounded-full hover:bg-slate-100 cursor-pointer text-slate-400 hover:text-slate-700 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 mb-6 space-y-2 border border-slate-100 text-xs sm:text-sm text-slate-700">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {isRtl
                      ? "تشخيص دقيق مع كادر استشاري مؤهل"
                      : "Accurate diagnosis with qualified consultants"}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-teal-600 shrink-0" />
                  <span>
                    {isRtl
                      ? "حجز فوري بدون انتظار في المستشفى"
                      : "Instant digital appointment with zero wait time"}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-teal-600 shrink-0" />
                  <span>
                    {isRtl
                      ? "متابعة نتائج الفحوصات عبر الملف الطبي"
                      : "Digital medical record integration"}
                  </span>
                </div>
              </div>
            </motion.div>
